...
static func build(parent: Node3D, terrain: Terrain, wrng: RandomNumberGenerator,
	...
	# Apron top sits just BELOW the ramp's upper end: its edge once stood
	# ~10 cm proud of the ramp, which at capsule radius is a >45° contact —
	# a wall — so leaving the cave needed a jump. Now it's a step down on
	# the way out (and not coplanar with the terrain either).
	Util.box(b, Vector3(6.6, 0.3, 3.8), Vector3(0, -0.18, 5.1), stone)
